...
| `rnidbg jni --so tests/fixtures/arm64/libhwdetect.so --call Java_…_runHardwareBreakpointCheck` | Loads the NDK SO, calls the JNI export, and returns the JSON report (`maxScore:280`). Guest `usleep`/`nanosleep`/`sigaction` are hooked like `printf` so the parent can yield without resuming into libc's post-SVC `cmn`. JNI workers skip `__pthread_start`. Cooperative `fork` runs the ptrace helper (pipe handshake, `PTRACE_ATTACH`/`SEIZE`/`GETREGSET`/`SETREGSET`). UE4 self-check reaches stage 7 with 6 BP + 4 WP slots; overall **175/280**. SIGTRAP injection on BP/WP hits is still missing, so the hot/cold items stay 0. Host exit 0. |
...
- Cooperative guest threads; `usleep`/`nanosleep`/`sigaction` libc hooks Halt so siblings can run without resuming into a libc SVC epilogue; `fork` clones GPRs + a stack window and runs the child as a task; pipe/`ppoll` can park briefly; deadlock recovery unlocks a contended mutex left by an exited thread
